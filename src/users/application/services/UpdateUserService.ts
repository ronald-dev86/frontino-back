   ...
   async  updateUser(id: string, payload: UpdateUserDTO):Promise<any> {
        ...
        for (const key in userFound[0]) {
            if (Object.prototype.hasOwnProperty.call(userFound[0], key)) {
                ...
                if (key === "password" && payload[key as keyof UpdateUserDTO] !== undefined) {
                    const newpassword = await this.keyOperator.encrypt(payload[key as keyof UpdateUserDTO]?.toString() || "");
                    userUpdate = {...userUpdate, [key]: newpassword};
                    continue
                }
        ...
        const user = new User(
            ...
            userUpdate.password || userFound[0].password,
            userUpdate.rol || userFound[0].rol,
